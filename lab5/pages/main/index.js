...
export class MainPage {
    ...
    async handleEdit(artist) {
        const modal = new ModalComponent(this.parent);
        modal.render(artist, async (updatedArtist) => {
            try {
                const { data, status } = await ajax.patch(artistUrls.updateArtistById(artist.id), updatedArtist);
                if (status === 200) {
                    this.renderArtists();
                } else {
    ...
    async handleDelete(artistId) {
        if (confirm('Вы уверены, что хотите удалить этого артиста?')) {
            try {
                const { status } = await ajax.delete(artistUrls.removeArtistById(artistId));
                if (status === 200) {
                    this.renderArtists();
                } else {
    ...
    async handleAdd() {
        ...
        modal.render(null, async (newArtist) => {
            try {
                const { data, status } = await ajax.post(artistUrls.createArtist(), newArtist);
                if (status === 201) {
                    this.renderArtists();
                } else {
